...
| **iCloud-offloaded attachments** | Distinguish (and eventually download) attachments Messages has offloaded to iCloud | S (label) / M (download) | ✅ 🚢 label · 🚫 download | **Labeling shipped.** `LiveIMessageProvider.attachment` now maps a chat.db row that named a real received file (path + non-zero `total_bytes`) whose local copy is gone to `.downloadRequired` instead of `.missing`; the file row reads "In iCloud — open in Messages to download" (neutral, not red) and the gallery/library tiles say "In iCloud" instead of "Unavailable". Display-only over the read-only path (tested in `LiveIMessageTests`). **Auto-download declined:** no in-charter, library-free trigger exists — `brctl download`/`evict` were removed from modern macOS, `FileManager.startDownloadingUbiquitousItem`/`NSFileProviderManager` only cover iCloud *Drive* items (Messages attachments aren't a File Provider domain), and Messages' AppleScript has no download verb. The only programmatic trigger is IMCore/`imagent` internals, which is exactly the janky `MessagesController` path `platform-imessage` wraps — a weak case for instantiating the gated library on its own. |
...
| **Undo send window** | Buffer the dispatch a few seconds so an accidental send can be cancelled | S | ✅ 🚢 | Shipped. `ComposerModel.send()` holds the message for a 5s window (opt-out via the `undoSend` setting, default on) instead of dispatching straight away. The box clears the instant you send and stays live for the next message; the held message rides in a floating `UndoSendToast` above the composer, with a depleting progress bar that dispatches the real `sendAction` when it empties. Undo (the toast's button or Esc / `.cancelAction`) cancels and hands the text back into the box; clicking the toast sends immediately. Switching conversations mid-window flushes the held send in the background (draft restored on failure) so a send is never dropped. The toast chrome is a reusable `ToastCard` (`DesignSystem/Components.swift`). Setting lives in Settings → Undo send. |
...
| **Beeper read-only adapter** | REST client against a headless Beeper Server for non-iMessage networks | L | ✅ 🚢 | Shipped and inert — no token, no provider. Built against the official SDK's published types and **never run against a live Server**; `scripts/beeper-contract-check.sh` would close that in one command. Track frozen 2026-08-04. |
| **Beeper connection settings** | Paste a token, set the endpoint, see per-provider health | M | 🚫 | §3, **frozen 2026-08-04** — not planned. Without it the token goes in the Keychain by hand. |
| **Beeper sends / WebSocket** | Send, react, mark-read gated per account; live socket ahead of the poll | L | 🚫 | §4, **frozen 2026-08-04** — not planned. |
| **Partial-failure banner** | Surface `partialFailures` / `degraded` in the sidebar instead of only logging | S | 🔨 | Carried by the model already. Was to land with the Beeper settings (now frozen); still worth it on its own, since the composite reports partials whatever the children are. |
